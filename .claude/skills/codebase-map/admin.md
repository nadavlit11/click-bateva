...
- `app/src/admin/components/EnrichModal.tsx` — POI enrichment from website: calls `enrichPoiFromWebsite` Cloud Function, shows cherry-pick UI (checkboxes for scalars incl. description, image grid 2-col, opening hours, location with client-side geocoding from address), email display-only, per-field thumbs up/down feedback, free-text notes, saves feedback to `enrichment_feedback` collection, triggers `updateEnrichmentInstructions` for self-improving prompts. All fields always shown ("לא נמצא" for empty). Modal uses `maxWidth="2xl"` for larger image preview. Exports `ApplyFields` interface (extended with `minPeople`/`maxPeople`).
- `app/src/admin/components/EnrichFromDescriptionModal.tsx` — POI enrichment from the POI's own description text: calls `enrichPoiFromDescription` callable, shows phone/whatsapp/price/minPeople/maxPeople/openingHours with before/after comparison, plus `cleanedDescription` (unchecked by default — requires intentional review). Mirrors EnrichModal structure but no images/videos/location. Saves feedback to `enrichment_feedback` with `source: 'description'`.

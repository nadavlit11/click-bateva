...
- `app/src/admin/components/EnrichModal.tsx` — POI enrichment from website: calls `enrichPoiFromWebsite` Cloud Function, shows cherry-pick UI (checkboxes for scalars, image grid, opening hours table), per-field thumbs up/down feedback, free-text notes, saves feedback to `enrichment_feedback` collection, triggers `updateEnrichmentInstructions` for self-improving prompts

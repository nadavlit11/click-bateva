...
> - **New fields on existing Firestore documents must be optional:** When adding a field to a TypeScript interface that maps to a Firestore collection with existing documents, the field must be typed as optional (`field?: string`) and all read-sites must guard against `undefined` (`data.field || ''`). Existing docs won't have the field — using a required type hides the runtime `undefined` from TypeScript, causing `.trim()` / `.toLowerCase()` crashes.
> - **CRM Firestore listeners must use `useAuthEffect`:** Every `onSnapshot` call in the CRM app (`crm/src/`) must use the `useAuthEffect` hook from `crm/src/hooks/useAuthSnapshot.ts` — never a raw `useEffect`. A `useEffect` with `[]` deps fires before Firebase auth resolves, causing permission-denied errors. `useAuthEffect` gates the subscription behind `authReady`. The callback should return the `onSnapshot` unsubscribe function for automatic cleanup.
>
> Output: PASS or FAIL, followed by a numbered list of findings (empty list if PASS).
>
> Diff:
